...
// options.https = { key: fs.readFileSync('keys/agent2-key.pem'), cert:
// fs.readFileSync('keys/agent2-cert.pem')};
// options.sleep = 3000;
...
function app() {
	...
	var directories = [ directory + "/controllers", directory + "/definitions",
			directory + "/isomorphic", directory + "/modules",
			directory + "/resources", directory + "/models",
			directory + "/source", directory + "/workers" ];
	var files = {};
	var force = false;
	var changes = [];
	var app = null;
	...
	function onComplete() {
		...
		fs.readdir(directory, function(err, arr) {
			...
			for (var i = 0; i < length; i++) {
				var name = arr[i];
				if (name === "config" || name === "config-debug"
						|| name === "config-release" || name === "versions"
						|| name === "sitemap" || name === "dependencies"
						|| name.indexOf(".js") !== -1
						|| name.indexOf(".resource") !== -1)
					self.file.push(name)
			}
